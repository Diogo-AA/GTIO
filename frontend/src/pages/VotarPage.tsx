...
export default function VotarPage() {
  ...
  if (loading) return (
    <div className="container">
      <div className="loading-state"><div className="spinner" /><p>{t('common.cargando')}</p></div>
    </div>
  )

  if (error || !gala) return (
    <div className="container">
  ...
  return (
    <div className="container">
      <div className="flex-between" style={{ marginBottom: '.75rem' }}>
        <div>
          <div className="section-title" style={{ marginBottom: '.1rem' }}>{gala.nombre}</div>
          <div style={{ fontSize: '.83rem', color: 'var(--gray-400)' }}>
            {fecha} · {t('votar.votosTotales', { n: totalVotos })}
          </div>
        </div>
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('/galas')}>{t('votar.volver')}</button>
      </div>

      <div className="box">
        <div className="section-title" style={{ marginBottom: '.75rem' }}>{t('votar.candidatos')}</div>
        {(gala.candidatos || []).map((c, idx) => {
          ...
          return (
            <div
              ...
              style={{ borderBottom: idx === gala.candidatos.length - 1 ? 'none' : undefined }}
            >
      ...
      <div className="box" style={{ marginTop: '.5rem' }}>
        <div className="section-title" style={{ marginBottom: '.75rem' }}>{t('votar.resultados')}</div>
        {sorted.map(c => {
          const pct = totalVotos > 0 ? Math.round((c.numVotos / totalVotos) * 100) : 0
          return (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: '.75rem', marginBottom: '.75rem' }}>
              <div style={{ width: 130, fontSize: '.85rem', fontWeight: 600, flexShrink: 0 }}>{c.nombre}</div>
              <div style={{ flex: 1 }}>
                <div className="progress-bg" style={{ maxWidth: '100%' }}>
                  <div className="progress-fill" style={{ width: `${pct}%` }} />
                </div>
              </div>
              <div style={{ width: 80, fontSize: '.8rem', color: 'var(--gray-400)', textAlign: 'right' }}>
                {c.numVotos} ({pct}%)
              </div>
            </div>
          )
