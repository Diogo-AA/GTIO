...
test('renders title and login tab active by default', async ({ page }) => {
  ...
  await expect(page.locator('.tabs button.tab.active')).toHaveText(t('login.iniciarSesion'))
...
test('switches to register tab and shows register form', async ({ page }) => {
  await page.goto('/login')
  await page.locator('.tabs button', { hasText: t('login.crearCuenta') }).click()
  await expect(page.locator('.tabs button.tab.active')).toHaveText(t('login.crearCuenta'))
...
test('switching back to login tab hides register form', async ({ page }) => {
  await page.goto('/login')
  await page.locator('.tabs button', { hasText: t('login.crearCuenta') }).click()
  await page.locator('.tabs button', { hasText: t('login.iniciarSesion') }).click()
  await expect(page.locator('.tabs button.tab.active')).toHaveText(t('login.iniciarSesion'))
  await expect(page.locator('input#login-username')).toBeVisible()
...
test('shows toast when passwords do not match on register', async ({ page }) => {
  await page.goto('/login')
  await page.locator('.tabs button', { hasText: t('login.crearCuenta') }).click()
  ...
  await expect(page.locator('.toast-msg')).toContainText(t('login.contrasenasNoCoinciden'))
})

test('login form has correct inputs and submit button', async ({ page }) => {
  await page.goto('/login')
  await expect(page.locator('input#login-username')).toHaveAttribute('type', 'text')
  await expect(page.locator('input#login-password')).toHaveAttribute('type', 'password')
  await expect(page.locator('button[type="submit"]')).toHaveText(t('login.entrar'))
})
