...
const mockDetalle = {
  ...
  votos: [
    ...
    { gala: { nombre: 'Gala 2' }, candidato: { nombre: 'Candidato B' }, fecha: '2026-02-20T00:00:00Z' },
  ],
}
...
test.describe('PerfilPage', () => {
  test('muestra estado de carga', async ({ page }) => {
    ...
    await page.route(API_PERFIL, skipDocument(async route => {
      await blocker
    ...
    await expect(page.locator('.loading-state p')).toHaveText(t('perfil.cargando'))
    unblock()
    await expect(page.locator('.loading-state')).not.toBeVisible()
  })

  test('muestra historial vacío', async ({ page }) => {
    await page.route(API_PERFIL, skipDocument(route =>
      route.fulfill({ json: { ...mockDetalle, votos: [] } })
    ))
    await loginDemo(page)
    await page.goto('/perfil')
    await expect(page.locator('.empty-state h3')).toHaveText(t('perfil.sinVotos'))
    await expect(page.locator('.empty-state p')).toHaveText(t('perfil.sinVotosMensaje'))
  })

  test('muestra tabla con votos', async ({ page }) => {
    await page.route(API_PERFIL, skipDocument(route =>
      route.fulfill({ json: mockDetalle })
    ))
    await loginDemo(page)
    await page.goto('/perfil')
