...
const mockDetalle = {
  ...
  username: 'alice',
  votos: [
    { gala: { nombre: 'Gala 1' }, candidato: { nombre: 'Candidato A' }, fecha: '2026-01-15T00:00:00Z' },
  ],
}
...
test.describe('UsuariosPage', () => {
  test('muestra lista de usuarios', async ({ page }) => {
    await page.route(API_USUARIOS, skipDocument(route =>
      route.fulfill({ json: mockUsuarios })
    ))
  ...
  test('muestra estado vacío cuando no hay usuarios', async ({ page }) => {
    ...
    await loginDemo(page)
    await page.goto('/usuarios')
    await expect(page.locator('.empty-state')).toContainText(t('usuarios.sinUsuarios'))
  })

  test('muestra error cuando falla la API', async ({ page }) => {
    await page.route(API_USUARIOS, skipDocument(route => route.abort()))
    await loginDemo(page)
    await page.goto('/usuarios')
  ...
  test('abre modal al clicar ver historial', async ({ page }) => {
    await page.route(API_USUARIOS, skipDocument(route => route.fulfill({ json: mockUsuarios })))
    await page.route('**/usuarios/1', skipDocument(route => route.fulfill({ json: mockDetalle })))
    await loginDemo(page)
    await page.goto('/usuarios')
    await page.locator('button', { hasText: t('usuarios.verHistorial') }).first().click()
  ...
  test('muestra sin votos si el usuario no ha votado', async ({ page }) => {
    await page.route(API_USUARIOS, skipDocument(route => route.fulfill({ json: mockUsuarios })))
    await page.route('**/usuarios/1', skipDocument(route =>
      route.fulfill({ json: { id: 1, username: 'alice', votos: [] } })
    ))
    await loginDemo(page)
    await page.goto('/usuarios')
    await page.locator('button', { hasText: t('usuarios.verHistorial') }).first().click()
    await expect(page.locator('.modal p', { hasText: t('modal.sinVotos') })).toBeVisible()
  })
})
