...
export default function Navbar() {
  ...
  const visibleLinks = NAV_LINKS.filter(link =>
    link.role === null || user?.roles.includes(link.role)
  )

  return (
    <nav>
      <div className="nav-brand" style={{ cursor: 'pointer' }} role="button" tabIndex={0} onClick={() => navigate(isAuthenticated ? '/galas' : '/login')} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') navigate(isAuthenticated ? '/galas' : '/login') }}>
        {t('nav.brand')}
      </div>
      ...
      <div id="nav-actions">
        ...
        {isAuthenticated && user ? (
          <>
            ...
            <button className="btn btn-ghost btn-sm" onClick={handleLogout}>{t('nav.salir')}</button>
          </>
        ) : (
          <>
